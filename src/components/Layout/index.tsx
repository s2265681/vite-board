...
import { useHistory, useLocation, Link } from 'dva';
...
const App: FC = ({ children }: PropsWithChildren<any>) => {
  ...
  const renderRightUser = () => {
    ...
    let currentPath = window.location.pathname || '', renderStat={}
    ...
    if(/overview/.test(currentPath)){
      ...
      extra:[
        <Button key="3">加入会议</Button>,
        <Button key="2" type="primary">新建会议</Button>
      ]
    }}
    
    if(currentPath == '/login') renderStat = {
       menuRender: false,
       headerRender:false
     }
  return (
